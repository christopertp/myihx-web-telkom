...
pipeline {
    parameters {
        ...
        string(name: 'DOCKER_IMAGE_NAME',          description: 'Docker Image Name',                    defaultValue: 'myihx-web')

        string(name: 'CHAT_ID',                    description: 'chat id of telegram group',            defaultValue: '-1001463248493')
    }
    agent none
    options {
                ...
                stage('Unit Test') {
                    ...
                    steps {
                        ...
                        script {
                            ...
                            def node = tool name: 'NodeJS-12.16.0', type: 'jenkins.plugins.nodejs.tools.NodeJSInstallation'
                ...
                stage('Build') {
                    ...
                    steps {
                        ...
                        script{
                            ...
                            sh "docker build --build-arg ARGS_NODE_BUILD=${envStage.toLowerCase()}  --rm --no-cache --pull -t ${params.DOCKER_IMAGE_NAME}:${BUILD_NUMBER}-${commitId} ."
        }   }   }   }   }
        stage ('Deployment'){
            steps{
